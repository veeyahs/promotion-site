...
export function ROISection() {
  return (
    <section id="roi-inner" className="relative py-12 md:py-16">
      <div className="max-w-6xl mx-auto px-6">
        {/* Urgency banner */}
        <div className="reveal glass-card p-5 md:p-6 mb-8 text-center border-gold-accent/20">
          <p className="text-slate-400 text-xs mb-1">为什么现在就要行动？</p>
          <p className="text-base md:text-lg text-white">
            <span className="gold-text font-bold text-xl md:text-2xl">80%</span> 的头部企业已部署AI智能体并产生可衡量的经济回报
          </p>
        ...
        <div className="reveal mb-8">
          <h3 className="font-heading text-xl font-bold text-white mb-5 text-center">效率提升对比</h3>
          <div className="glass-card overflow-hidden">
            <div className="grid grid-cols-4 gap-4 p-4 border-b border-white/10 bg-ocean-dark/50">
            ...
            {roiData.map((row, index) => (
              <div key={index} className={`grid grid-cols-4 gap-4 p-4 ${index < roiData.length - 1 ? 'border-b border-white/5' : ''}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {promises.map((promise, index) => {
            ...
            return (
              <div key={index} className="reveal glass-card p-5 text-center" style={{ transitionDelay: `${index * 0.15}s` }}>
                <div className={`w-10 h-10 mx-auto rounded-full ${bgClass} flex items-center justify-center mb-3`}>
                  <CheckCircle className={`w-5 h-5 ${colorClass}`} />
                </div>
                <h4 className={`font-heading font-bold ${colorClass} mb-1`}>{promise.title}</h4>
                <p className="text-slate-400 text-sm">{promise.desc}</p>
              </div>
            );
          })}
        </div>
